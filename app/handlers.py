...
async def cancel(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f'💸 Доброго времени суток, {callback.from_user.first_name}!\n\nУправляйте счетами по кнопкам ниже. 👇',
        reply_markup=kb.main_kb)
...
async def registration_name(message: types.Message, state: FSMContext):
    if len(message.text) > 18:
        ...
        await message.answer(f'✅ Отличное название!\n\nВыберите, в какой валюте будет счёт {message.text}? 👇',
                             reply_markup=await kb.all_currencies_kb())
...
async def registration_done(callback: types.CallbackQuery, state: FSMContext):
    ...
    if await set_account_db(reg_data):
        await callback.message.edit_text(
            f'✅ Регистрация успешно завершена!\n\nТеперь Вы можете управлять своими счетами по кнопкам ниже.',
            reply_markup=kb.main_kb)
        await state.clear()
    else:
        await callback.message.answer('❌ Произошла ошибка... Обратитесь к администратору @rlxrd',
                                      reply_markup=kb.main_kb)
        await state.clear()
...
async def registration_canceled(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not user_reg:
        await state.set_state(Registration.name)
        await callback.message.answer('🤔 Введите название счёта. Например: Основной.',
                                      reply_markup=ReplyKeyboardRemove())
    else:
        await callback.message.answer(
            f'💸 Доброго времени суток, {callback.from_user.first_name}!\n\nУправляйте счетами по кнопкам ниже. 👇',
...
async def topup(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        print(err)
    await callback.message.answer('🔒 Выберите счёт для <b>пополнения</b>.',
                                  reply_markup=await kb.users_accounts_kb(callback.from_user.id))
...
async def edit_my_acc(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            f'Вы выбрали счёт: {account.name}\nБаланс: {account.balance} {cur.name}\nID: {account.id}',
            reply_markup=await kb.acc_settings(acc_id))
    except:
...
async def spend(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        print(err)
    await callback.message.answer('🔒 Выберите счёт для <b>расхода</b>.',
                                  reply_markup=await kb.users_accounts_kb(callback.from_user.id))
