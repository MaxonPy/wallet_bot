...
async def set_account_db(reg_data):
    async with async_session() as session:
        try:
            ...
            session.add(
                Accounts(name=reg_data['name'], balance=reg_data['amount'], currency=currency_d.id, user=user_d.id))
            await session.commit()
            return True
        except:
            return False
...
async def update_balance(amount, account, direction, type):
    async with async_session() as session:
        ...
        session.add(Transaction(date=datetime.now().strftime("%Y-%m-%d %H:%M"), amount=amount, account=account_data.id,
                                direction=direction_data.id, type=type))
        await session.execute(update(Accounts).where(Accounts.id == account_data.id).values(
            balance=(account_data.balance + float(amount)) if type else (account_data.balance - float(amount))))
        await session.commit()
...
async def fetch_my_accounts_db(tg_id):
    async with async_session() as session:
        user_d = await session.scalar(select(Users).where(Users.tg_id == tg_id))
        accounts_d = await session.execute(
            select(Accounts.id, Accounts.user, Accounts.name, Accounts.balance, Accounts.currency).where(
                Accounts.user == user_d.id))
    return accounts_d


async def delete_acc(acc_id, session):
    async with async_session() as session:
    ...
    async with async_session() as session:
        ...
        for account in accounts:
            ...
            transactions = await session.scalars(
                select(Transaction).where(Transaction.account == account.id).order_by(desc(Transaction.id)))
            for transaction in transactions:
                transactions_all[
                    transaction.id] = f'{"🟢" if transaction.type else "🔴"} {transaction.date[-14:]} | {transaction.amount} {currency}'
